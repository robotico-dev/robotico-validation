...
/// <summary>
/// Tests that ValidationErrors and Builder obey expected invariants: additive builder, round-trip, success when no errors.
/// </summary>
public sealed class ValidationLawsTests
{
    ...
    public void Builder_ToResult_failure_when_any_errors_added()
    {
        ...
        Assert.True(r.IsError(out Robotico.Result.Errors.IError? err));
    ...
    public void Builder_ToDictionary_Then_ToResult_produces_same_validation_error_shape()
    {
        ...
        Robotico.Result.Result r = ValidationErrors.ToResult(dict, "Summary", "CODE");
        Assert.False(r.IsSuccess());
    ...
    {
        Robotico.Result.Result r = ValidationErrors.ForField("email", "Invalid.");
        Assert.False(r.IsSuccess());
        r.IsError(out Robotico.Result.Errors.IError? err);
        ValidationError ve = Assert.IsType<ValidationError>(err);
